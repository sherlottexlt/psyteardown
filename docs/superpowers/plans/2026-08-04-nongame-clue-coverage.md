...
逐条剔除实验证明它是**唯一**的肇因(其余 13 条全留、只去掉这一条即全绿),且改写为不含 `课程` 的同义写法(`内容解锁` / `逐级解锁` / `章节解锁` / `进度解锁`)同样全绿。故取 `内容解锁`:保住「解锁」这个语义,避开 `课程` 这个碰撞词元。冥想画像仍达标,因为它另有 `逐级进阶`→`逐级`、`时长统计`→`时长/统计`、`课程解锁`原本贡献的`解锁`由`内容解锁`接住。
...
principles:
  - id: autonomy
    ...
    look_for: [可自定义, 可跳过, 非强制路径, 自选内容, 跳过引导, 免打扰, 自定义时长]
  - id: competence
    name: 胜任(Competence)
    description: 用户感到自己在进步、能掌控。
    look_for: [难度梯度, 即时反馈, 技能提升曲线, 内容解锁, 逐级进阶, 关卡, 完成率, 时长统计]
  - id: relatedness
    name: 归属(Relatedness)
    description: 用户感到与他人连接。
    look_for: [社区, 协作, 互助, 关系绑定, 好友, 一起, 同伴, 组队, 分享给朋友]
```

共 13 条新线索(autonomy 4 / competence 5 / relatedness 5,减去收回的 `课程解锁`)。

**「内容解锁」不得写成「课程解锁」**——理由见本文件开头「前置」第一条,写成 `课程解锁` 会让 Task 3 的既有案例护栏失败。
...
Expected: 3 passed。冥想画像头名变为 `self-determination-theory`(分数 15.94,非零 7/8)。
...
def test_existing_cases_keep_their_head_framework():
    ...
    已知结论(实现时实测):13 条线索全绿。若把 competence 的「内容解锁」写成
...
Expected: `N=8 词元总数=426 df直方图={1: 392, 2: 28, 3: 6}`,`IDF 区间 0.981~2.079`。
...
|------|------|------|
| 全库词元总数 | 403 | 426 |
| df 直方图 | `{1: 373, 2: 24, 3: 6}` | `{1: 392, 2: 28, 3: 6}` |
| IDF 区间 | 0.981~2.079 | 0.981~2.079 |

**稀释未发生。** df 上限仍为 3,IDF 区间逐位不变;新增的 23 个词元里 19 个是 df=1
的独有词元,只有 4 个把某词元从 df=1 抬到 df=2。原因是补入的表层词汇本就不在其他
...
```markdown
> **实现修正:`课程解锁` 已收回,改写为 `内容解锁`。** 它引入词元 `课程`,与 DB 中
